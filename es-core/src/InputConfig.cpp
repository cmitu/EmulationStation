...
InputConfig::InputConfig(int deviceId, const std::string& deviceName, const std::string& deviceGUID, const unsigned int vendorId=0, const unsigned int productId=0) : mDeviceId(deviceId), mDeviceName(deviceName), mDeviceGUID(deviceGUID), mVendorId(vendorId), mProductId(productId)
{
}
...
void InputConfig::writeToXML(pugi::xml_node& parent)
{
	...
	if(mCompatDeviceName != mDeviceName && ! mCompatDeviceName.empty() )
		cfg.append_attribute("compatDeviceName") = mCompatDeviceName.c_str();
